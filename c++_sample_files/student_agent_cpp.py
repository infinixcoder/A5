...
class StudentAgent(BaseAgent):
    ...
    def choose(self, board: List[List[Any]], rows: int, cols: int, score_cols: List[int], current_player_time: float, opponent_time: float) -> Optional[Dict[str, Any]]:
        ...
        # Make the board from list of list of piece to list of list of dictonary
        board_dict = [[{} for _ in range(cols)] for _ in range(rows)]
        for r in range(rows):
            for c in range(cols):
                if board[r][c] is not None:
                    board_dict[r][c] = {
                        "owner": str(board[r][c].owner),
                        "side": str(board[r][c].side),
                        "orientation": str(board[r][c].orientation)
                    }
        
        cpp_move = self.agent.choose(board_dict, rows, cols, score_cols, current_player_time, opponent_time)
        if cpp_move.action == "" :
            print("bkchdi")
            return None
...
def test_student_agent():
    ...
    try:
        ...
        move = agent.choose(board_dict, rows, cols, score_cols,1.0,1.0)
